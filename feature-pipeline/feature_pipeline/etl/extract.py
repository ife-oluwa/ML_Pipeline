...
def from_api(
    ...
    Args:
        export_end_reference_datetime The end reference datetime of the export window. If None, the current time is used.
            ...
            The real extracted window will be computed as [export_end_reference_datetime -daays_delay -days_export, export_end_refernce_datetime - days_delay],

        days_delay (int, optional): Data has a delay of N days, we have to shift our window with N days. Defaults to 15.
        days_export (int, optional): The number of days to export. Defaults to 30.
        url (_type_, optional): URL of the API. Defaults to "https://api.energidataservice.dk/dataset/ConsumptionDE35Hour".

    Returns:
        Optional[Tuple[pd.DataFrame, Dict[str, Any]]]: A tuple of a Pandas Data Frame containing the exported data and a dictionary of metadata.
    """
    ...
    logger.info(f"Response recieved from API with status code: {response.status_code}"),
