...
def load_dataset_from_feature_store(
    ...
    Args:
        feature_view_version (int): feature store feature view version to load data
        training_dataset_version (int): feature store training dataset version to load data
        fh (int, optional): Forecast horizon. Defaults to 24.

    Returns:
        Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]: Train and test splits loaded from the feature store as pandas dataframes.
    """
    ...
    with init_wandb_run(
        ...
        raw_data_at = wandb.Artifact(
            name="energy_consumption_feature_view",
            type="feature_view",
...
def prepare_data(
    ...
    # Set the index as is required by sktime
    data["datatime_utc"] = pd.PeriodIndex(data["datatime_utc"], freq="H")
