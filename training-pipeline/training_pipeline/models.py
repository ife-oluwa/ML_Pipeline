...
def build_model(config: dict):
    ...
    lag = config.pop(
        "forecaster_transformers__widow_summarizer__lag_feature__lag",
        list(range(1, 72 +1)),
    )
    mean = config.pop(
        "forecaster_transformers__widow_summarizer__lag_feature__mean",
        [[1, 24], [1, 48], [1, 72]],
    )

    std = config.pop(
        "forecaster_transformers__widow_summarizer__lag_feature__std",
        [[1, 24], [1, 48], [1, 72]],
    )
